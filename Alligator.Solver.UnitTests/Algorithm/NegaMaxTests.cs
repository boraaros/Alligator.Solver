...
namespace Alligator.Solver.UnitTests.Algorithm
{
    ...
    public class NegaMaxTests
    {
        ...
        public void Negamax_does_not_call_static_evaluation_for_intermediate_nodes()
        {
            ...
            var heuristicTables = new Mock<IHeuristicTables<TestPly>>(MockBehavior.Loose);

            var externalLogics = new Mock<IExternalLogics<TestPosition, TestPly>>(MockBehavior.Strict);
            externalLogics
        ...
        public void Negamax_add_transpositions_to_cache()
        {
            ...
            var cacheTables = new Mock<ICacheTables<TestPosition, TestPly>>(MockBehavior.Loose);
            cacheTables
                .Setup(t => t.AddTransposition(It.IsAny<TestPosition>(), It.IsAny<Transposition<TestPly>>()))
                .Callback<TestPosition, Transposition<TestPly>>((p, t) => idsToCache.Add(p.Identifier));
